...
namespace CuCrypto
{
	...
	class Crc32 : public IHashAlgorithm<Crc32, std::uint32_t>
	{
	public:
		using HashValueType = HashValueType;

		Crc32() : data(~std::uint32_t{0}) {}
	...
	template <>
	inline Hash<Crc32::HashValueType>::Hash(const Crc32::HashValueType &val) : Data(val)
	{
	}

	template <>
	void Hash<unsigned>::ToString(std::string &buf) const
	{
		Detail::Hash::ToStringPaddingZero(Data, buf);
	}
